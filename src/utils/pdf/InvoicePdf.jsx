...
const InvoicePdf = async (invoiceData, projectName, poNumber, description, setIsLoading) => {
  try {
    setIsLoading(true);
    if (!invoiceData || invoiceData.length === 0) {
      throw new Error("No Invoice SD RM Release data available");
    }
    ...
    const addHeader = () => {
      ...
      // Subtitle - Invoice SD RM Release
      const subTitle = "Invoice SD RM Release";
      doc.setFontSize(12);
    ...
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("Invoice SD RM Release Details", 14, lastTableY);
    lastTableY += 15;
    ...
    // Prepare table data
    const tableData = invoiceData.map((doc, index) => [
      (index + 1).toString(),
      doc.docName || "N/A",
    ...
    const filename = `${safeProjectName} - Invoice SD RM Release - ${formattedDateTime}.pdf`;

    doc.save(filename);
  } catch (error) {
    console.error("Error generating Invoice SD RM Release PDF:", error);
    alert(`Failed to generate Invoice SD RM Release PDF: ${error.message}`);
  } finally {
    setIsLoading(false);
  }
};
