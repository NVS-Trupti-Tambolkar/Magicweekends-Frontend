...
const PO_SDpdf = async (sdData, projectName, poNumber, description, setIsLoading) => {
  try {
    setIsLoading(true);
    if (!sdData || sdData.length === 0) {
      throw new Error("No Security Deposit data available");
    }
    ...
    const addHeader = () => {
      ...
      const subTitle = "Security Deposit";
      doc.setFontSize(12);
    ...
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("Security Deposit Details", 14, lastTableY);
    lastTableY += 15;
    ...
    // Prepare table data
    const tableData = sdData.map((doc, index) => [
      (index + 1).toString(),
      doc.docName || "N/A",
    ...
    const filename = `${safeProjectName} - Security Deposit - ${formattedDateTime}.pdf`;
    doc.save(filename);
  } catch (error) {
    console.error("Error generating Security Deposit PDF:", error);
    alert(`Failed to generate Security Deposit PDF: ${error.message}`);
  } finally {
    setIsLoading(false);
  }
};

export default PO_SDpdf;