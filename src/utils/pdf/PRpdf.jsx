...
const PRFormPdf = async (prData, projectName, poNumber, description, setIsLoading) => {
  try {
    setIsLoading(true);
    if (!prData) {
    ...
    const addHeader = () => {
      ...
      const subTitle = "Purchase Requisition";
      doc.setFontSize(12);
    ...
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("PR Details", 14, lastTableY);
    lastTableY += 15;
    ...
    const filteredData = Object.entries(prData)
    ...
    const filename = `${safeProjectName} - Purchase Requisition - ${formattedDateTime}.pdf`;
    doc.save(filename);
  } catch (error) {
    console.error("Error generating PR PDF:", error);
    alert(`Failed to generate PR PDF: ${error.message}`);
  } finally {
    setIsLoading(false);
  }
};

export default PRFormPdf;