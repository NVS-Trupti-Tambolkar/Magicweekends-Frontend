...
const AftersalesPdf = async (afterSalesData, projectName, poNumber, description, setIsLoading) => {
  try {
    setIsLoading(true);
    if (!afterSalesData || afterSalesData.length === 0) {
      throw new Error("No After Sales Complaints data available");
    }
    ...
    const addHeader = () => {
      ...
      // Subtitle - After Sales SR Complaints
      const subTitle = "After Sales SR Complaints";
      doc.setFontSize(12);
    ...
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("After Sales SR Details", 14, lastTableY);
    lastTableY += 15;
    ...
    // Prepare table data
    const tableData = afterSalesData.map((doc, index) => [
      (index + 1).toString(),
      doc.docName || "N/A",
    ...
    const filename = `${safeProjectName} - After Sales SR Complaints - ${formattedDateTime}.pdf`;

    doc.save(filename);
  } catch (error) {
    console.error("Error generating After Sales SR Complaints PDF:", error);
    alert(`Failed to generate After Sales SR Complaints PDF: ${error.message}`);
  } finally {
    setIsLoading(false);
  }
};

export default AftersalesPdf;