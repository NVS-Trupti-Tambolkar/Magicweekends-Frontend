...
const WOpdf = async (workOrderData, projectName, poNumber, description, setIsLoading) => {
  try {
    setIsLoading(true);
    if (!workOrderData) {
    ...
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("Work Order Details", 14, lastTableY);
    lastTableY += 15;
    ...
    const filteredData = Object.entries(workOrderData)
    ...
    const filename = `${safeProjectName} - Work Order - ${formattedDateTime}.pdf`;
    doc.save(filename);
  } catch (error) {
    console.error("Error generating Work Order PDF:", error);
    alert(`Failed to generate Work Order PDF: ${error.message}`);
  } finally {
    setIsLoading(false);
  }
};
