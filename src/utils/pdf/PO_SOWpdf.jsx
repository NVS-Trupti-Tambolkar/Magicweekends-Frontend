...
const PO_SOWpdf = async (sowData, projectName, poNumber, description, setIsLoading) => {
  try {
    setIsLoading(true);
    if (!sowData || sowData.length === 0) {
      throw new Error("No Scope of Work data available");
    }
    ...
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("Scope of Work Details", 14, lastTableY);
    lastTableY += 15;
    ...
    // Prepare table data
    const tableData = sowData.map((doc, index) => [
      (index + 1).toString(),
      doc.docName || "N/A",
    ...
    const filename = `${safeProjectName} - Scope of Work - ${formattedDateTime}.pdf`;
    doc.save(filename);
  } catch (error) {
    console.error("Error generating Scope of Work PDF:", error);
    alert(`Failed to generate Scope of Work PDF: ${error.message}`);
  } finally {
    setIsLoading(false);
  }
};

export default PO_SOWpdf;