...
const MISCPdf = async (miscData, projectName, poNumber, description, setIsLoading) => {
  try {
    setIsLoading(true);
    if (!miscData || miscData.length === 0) {
      throw new Error("No MISC data available");
    }
    ...
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("MISC Details", 14, lastTableY);
    lastTableY += 15;
    ...
    // Prepare table data
    const tableData = miscData.map((doc, index) => [
      (index + 1).toString(),
      doc.docName || "N/A",
    ...
    const filename = `${safeProjectName} - MISC - ${formattedDateTime}.pdf`;

    doc.save(filename);
  } catch (error) {
    console.error("Error generating MISC PDF:", error);
    alert(`Failed to generate MISC PDF: ${error.message}`);
  } finally {
    setIsLoading(false);
  }
};

export default MISCPdf;