...
const SitepicsPdf = async (sitePicsData, projectName, poNumber, description, setIsLoading) => {
  try {
    setIsLoading(true);
    if (!sitePicsData || sitePicsData.length === 0) {
      throw new Error("No Site Pics data available");
    }
    ...
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("Site Pics Details", 14, lastTableY);
    lastTableY += 15;
    ...
    // Prepare table data
    const tableData = sitePicsData.map((doc, index) => [
      (index + 1).toString(),
      doc.docName || "N/A",
    ...
    const filename = `${safeProjectName} - Site Pics - ${formattedDateTime}.pdf`;

    doc.save(filename);
  } catch (error) {
    console.error("Error generating Site Pics PDF:", error);
    alert(`Failed to generate Site Pics PDF: ${error.message}`);
  } finally {
    setIsLoading(false);
  }
};

export default SitepicsPdf;