...
const HandoverPdf = async (handoverData, projectName, poNumber, description, setIsLoading) => {
  try {
    setIsLoading(true);
    if (!handoverData || handoverData.length === 0) {
      throw new Error("No Handover Documents data available");
    }
    ...
    const addHeader = () => {
      ...
      // Subtitle - Handover Documents
      const subTitle = "Handover Documents";
      doc.setFontSize(12);
    ...
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("Handover Documents Details", 14, lastTableY);
    lastTableY += 15;
    ...
    // Prepare table data
    const tableData = handoverData.map((doc, index) => [
      (index + 1).toString(),
      doc.docName || "N/A",
    ...
    const filename = `${safeProjectName} - Handover Documents - ${formattedDateTime}.pdf`;

    doc.save(filename);
  } catch (error) {
    console.error("Error generating Handover Documents PDF:", error);
    alert(`Failed to generate Handover Documents PDF: ${error.message}`);
  } finally {
    setIsLoading(false);
  }
};
