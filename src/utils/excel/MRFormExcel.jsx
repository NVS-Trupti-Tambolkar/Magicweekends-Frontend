...
const MRFormExcel = async (mrData, setLoading, triggerNotification, poNumber, projectName) => {
  try {
    setLoading(true);
    if (!mrData) {
      throw new Error("No MR data available for export");
    }
    
    // Create workbook and worksheet
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Material Requisition");
    ...
    worksheet.getCell("A2").value = "MATERIAL REQUISITION";
    ...
    // MR Form Table Header (Row 5)
    const mrHeaderRow = worksheet.addRow(["MR Details"]);
    worksheet.mergeCells("A5:G5");
    mrHeaderRow.getCell(1).font = { bold: true, size: 14, name: "Calibri" };
    mrHeaderRow.getCell(1).alignment = { horizontal: "left", vertical: "middle" };
    worksheet.getRow(5).height = 25;
    
    worksheet.addRow([]); // Spacer (Row 6)
    
    // MR Form Table Headers
    const mrTableHeaderRow = worksheet.addRow(["Field", "", "", "", "Value", "", ""]);
    ...
    // MR Form Data
    const mrDataRows = Object.entries(mrData)
    ...
    mrDataRows.forEach((rowData, index) => {
    ...
    const excelFileName = `${projectName || 'Project'} - Material Requisition - ${dateStr}.xlsx`;
