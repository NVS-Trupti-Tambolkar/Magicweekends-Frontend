// utils/excel/SitepicsExcel.js
import ExcelJS from "exceljs";
import ColorLogo from "../../assets/Excel/ColorLogo.png";

const SitepicsExcel = async (sitePicsData, setLoading, triggerNotification, projectName) => {
  try {
    setLoading(true);
    if (!sitePicsData || sitePicsData.length === 0) {
      throw new Error("No Site Pics data available for export");
    }

    // Create workbook and worksheet
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Site Pics");
    ...
    worksheet.getCell("A2").value = "SITE PICS";
    ...
    // Section Header: Site Pics Details
    const sectionHeader = worksheet.addRow(["Site Pics Details"]);
    ...
    // Add data rows
    sitePicsData.forEach((doc, index) => {
    ...
    const excelFileName = `${projectName || 'Project'} - Site Pics - ${dateStr}.xlsx`;
