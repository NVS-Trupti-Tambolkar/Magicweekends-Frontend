...
const PO_SOWExcel = async (sowData, setLoading, triggerNotification, poNumber, projectName) => {
  try {
    setLoading(true);
    if (!sowData) {
      throw new Error("No SOW data available for export");
    }
    
    // Create workbook and worksheet
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("PO Scope of Work");
    ...
    worksheet.getCell("A2").value = "PURCHASE ORDER SCOPE OF WORK";
    ...
    // SOW Table Header (Row 5)
    const sowHeaderRow = worksheet.addRow(["Scope of Work Details"]);
    worksheet.mergeCells("A5:G5");
    sowHeaderRow.getCell(1).font = { bold: true, size: 14, name: "Calibri" };
    sowHeaderRow.getCell(1).alignment = { horizontal: "left", vertical: "middle" };
    worksheet.getRow(5).height = 25;
    
    worksheet.addRow([]); // Spacer (Row 6)
    
    // SOW Table Headers
    const sowTableHeaderRow = worksheet.addRow(["Sr.No", "Document Name", "Document Reference", "File"]);
    worksheet.mergeCells("A7:A7"); // Sr.No header
    ...
    const excelFileName = `${projectName || 'Project'} - PO Scope of Work - ${dateStr}.xlsx`;
