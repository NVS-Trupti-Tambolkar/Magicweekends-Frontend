...
const WorkorderExcel = async (data, setLoading, onError, poNumber, projectName) => {
  try {
    setLoading(true);
    
    // Create workbook and worksheet
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Work Order Details");
    ...
    worksheet.getCell("A2").value = "WORK ORDER DETAILS";
    ...
    // Work Order Details Table Header (Row 5)
    const woHeaderRow = worksheet.addRow(["Work Order Details"]);
    worksheet.mergeCells("A5:G5");
    woHeaderRow.getCell(1).font = { bold: true, size: 14, name: "Calibri" };
    woHeaderRow.getCell(1).alignment = { horizontal: "left", vertical: "middle" };
    worksheet.getRow(5).height = 25;
    
    worksheet.addRow([]); // Spacer (Row 6)
    
    // Work Order Details Table Headers
    const woTableHeaderRow = worksheet.addRow(["Field", "", "", "", "Value", "", ""]);
    ...
    // Work Order Details Data
    const woDataRows = [
      ["Work Order No", data.WoNo || ""],
      ["Project No", data.ProjectNo || ""],
      ["Buyers PO No", data.BuyersPONo || ""],
      ["Product Details", data.ProductDetails || ""],
      ["System Config", data.SystemConfig || ""],
      ["Engineering Dept Person", data.EngineeringDeptPerson || ""],
      ["Project Engineer", data.ProjectEngineer || ""],
      ["Tele Dept Person", data.TeleDeptPerson || ""],
      ["TRD Dept Person", data.TrdDeptPerson || ""],
      ["Project Consignee Person", data.ProjectConsigneePerson || ""],
      ["Site Address", data.SiteAddress || ""],
      ["Scope Of Work", data.ScopeOfWork || ""],
      ["Material Required", data.MaterialRequired || ""],
      ["Quantity", data.Qty || ""],
      ["Start Date", data.StartDate ? new Date(data.StartDate).toLocaleDateString() : ""],
      ["End Date", data.EndDate ? new Date(data.EndDate).toLocaleDateString() : ""]
    ];
    
    woDataRows.forEach((rowData, index) => {
    ...
    const excelFileName = `${projectName || 'Project'} - Work Order - ${dateStr}.xlsx`;
