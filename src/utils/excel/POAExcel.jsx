...
const POAExcel = async (poaData, setLoading, triggerNotification, poNumber, projectName) => {
  try {
    setLoading(true);
    if (!poaData) {
      throw new Error("No POA data available for export");
    }
    
    // Create workbook and worksheet
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Power of Attorney");
    ...
    worksheet.getCell("A2").value = "POWER OF ATTORNEY";
    ...
    // POA Table Header (Row 5)
    const poaHeaderRow = worksheet.addRow(["Power of Attorney Details"]);
    worksheet.mergeCells("A5:G5");
    poaHeaderRow.getCell(1).font = { bold: true, size: 14, name: "Calibri" };
    poaHeaderRow.getCell(1).alignment = { horizontal: "left", vertical: "middle" };
    worksheet.getRow(5).height = 25;
    
    worksheet.addRow([]); // Spacer (Row 6)
    
    // POA Table Headers
    const poaTableHeaderRow = worksheet.addRow(["Sr.No", "Document Name", "Document Reference", "File"]);
    ...
    const excelFileName = `${projectName || 'Project'} - Power of Attorney - ${dateStr}.xlsx`;
