...
const POsummaryExcel = async (poData, setLoading, triggerNotification, projectName) => {
  try {
    setLoading(true);
    if (!poData) {
      throw new Error("No PO Summary data available for export");
    }
    ...
    worksheet.getCell("A2").value = "PURCHASE ORDER SUMMARY";
    ...
    // PO Summary Table Header (Row 5)
    const poHeaderRow = worksheet.addRow(["PO Summary Details"]);
    ...
    // PO Summary Table Headers - FIXED: Properly set headers with correct merging
    ...
    // PO Summary Data - Fixed to properly display values
    const poDataRows = Object.entries(poData)
      .filter(([key]) => !["ID", "POID", "Date opennessofCreation", "DateofModification"].includes(key))
      .map(([key, value]) => {
    ...
    const excelFileName = `${projectName || 'Project'} - PO Summary - ${dateStr}.xlsx`;
