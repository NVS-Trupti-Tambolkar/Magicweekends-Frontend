// utils/excel/DCExcel.js
import ExcelJS from "exceljs";
import ColorLogo from "../../assets/Excel/ColorLogo.png";

const DCExcel = async (dcData, setLoading, triggerNotification, projectName) => {
  try {
    setLoading(true);
    if (!dcData || dcData.length === 0) {
      throw new Error("No DC Schedule data available for export");
    }

    // Create workbook and worksheet
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("DC Schedule");
    ...
    worksheet.getCell("A2").value = "DC CA IR SCHEDULE";
    ...
    // Section Header: DC Schedule Details
    const sectionHeader = worksheet.addRow(["DC Schedule Details"]);
    ...
    const excelFileName = `${projectName || 'Project'} - DC Schedule - ${dateStr}.xlsx`;
