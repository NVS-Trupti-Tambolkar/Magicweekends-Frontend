// utils/excel/HandoverExcel.js
import ExcelJS from "exceljs";
import ColorLogo from "../../assets/Excel/ColorLogo.png";

const HandoverExcel = async (handoverData, setLoading, triggerNotification, projectName) => {
  try {
    setLoading(true);
    if (!handoverData || handoverData.length === 0) {
      throw new Error("No Handover data available for export");
    }

    // Create workbook and worksheet
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Handover Documents");
    ...
    worksheet.getCell("A2").value = "HANDOVER DOCUMENTS";
    ...
    // Section Header: Handover Documents Details
    const sectionHeader = worksheet.addRow(["Handover Documents Details"]);
    ...
    // Add data rows
    handoverData.forEach((doc, index) => {
    ...
    const excelFileName = `${projectName || 'Project'} - Handover Documents - ${dateStr}.xlsx`;
